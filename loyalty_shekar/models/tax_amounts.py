...
class SaleOrders(models.Model):
    ...
    points_in_words = fields.Char('Points in words',compute='do_points')
    # z_e_way_bill=fields.Char('E Way Bill',store=True)
    # z_number_of_items=fields.Integer("Number of items")
    z_today_date = fields.Date.today()
    z_inv_total_points=fields.Float("Total Points",store=True)

    @api.multi
    def action_invoice_open(self):
        res = super(SaleOrders, self).action_invoice_open()
        for line in self:
            var = 0
            val = line.env['loyalty.earning.report'].search([('invoice_id','=',line.id)\
                ,('status','=','open')])
            for l in val:
                var += l.points
...
# class Loyaltyproduct(models.Model):
#     _inherit = "account.invoice.line"

#     z_points = fields.Float("Reward points",compute="fetch_points",store="True")


#     @api.multi
#     def fetch_points(self):
#         for line in self:
#             var = line.env['loyalty.earning.report'].search(['product_id','=',line.product_id.id]).points
#             for val in var:
#                 line.z_points=val.points 
